...
levels(results$contrast.type)[levels(results$contrast.type) == "pcwsLinMean"] <- "\\ref{Scen:change_in_slope}"
levels(results$contrast.type)[levels(results$contrast.type) == "pcwsLinContMean"] <- "\\ref{Scen:change_in_mean_and_slope}"
...
ggplot(results, aes(x=n, y=time.solution.path, shape=contrast.type)) + geom_line()
...
pdf(file = paste("pdf/",file.name,".pdf", sep=""), width=comp.times.width, height=height)
print(pl)
dev.off()

tikz(file = paste("tikz/",file.name,".tex", sep=""),  width=comp.times.width, height=height)
print(pl)
dev.off()
